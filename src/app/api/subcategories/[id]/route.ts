...
export async function PATCH(
  ...
  try {
    ...
    const existing = db.prepare("SELECT * FROM subcategories WHERE id = ?").get(id);
    if (!existing) {
      return NextResponse.json({ error: "Subcategory not found" }, { status: 404 });
    }

    const updatableFields = ["name", "category_id", "sort_order"] as const;
    ...
    if (setClauses.length > 0) {
      ...
      db.prepare(
        `UPDATE subcategories SET ${setClauses.join(", ")} WHERE id = ?`
      ).run(...setParams, id);
      ...
      db.prepare(
        `INSERT INTO activity_log (id, action, entity_type, entity_id, user_id, description, created_at)
         VALUES (?, ?, ?, ?, ?, ?, ?)`
      ).run(logId, "updated", "subcategory", id, session.userId, description, now);
    }
