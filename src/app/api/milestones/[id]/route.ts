...
export async function PATCH(
  ...
  try {
    ...
    const updatableFields = ["title", "target_date", "description", "status"] as const;
    ...
    if (setClauses.length > 0) {
      ...
      const description = body.status === "complete"
        ? `Completed milestone`
        : `Updated milestone`;
      db.prepare(
        `INSERT INTO activity_log (id, action, entity_type, entity_id, user_id, description, created_at)
         VALUES (?, ?, ?, ?, ?, ?, ?)`
      ).run(logId, "updated", "milestone", id, session.userId, description, now);
    }
